...
expName = 'RunB_MRI_FaceExperiment'  # from the Builder filename that created this script
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath=u'C:\\Users\\SilversLab\\Desktop\\Faces Task\\MRI Task\\RunB_MRI_FaceExperiment.psyexp',
...
with open("RunB_Jitter.csv") as f:
...
condition_input_file = "RunB_ConditionsInputFile.csv"
...
condition_output_file = "RunB_ConditionsFile.csv"
...
# Initialize components for Routine "hwg"
hwgClock = core.Clock()
...
# ------Prepare to start Routine "hwg"-------
t = 0
hwgClock.reset()  # clock
frameN = -1
continueRoutine = True
routineTimer.add(4.000000)
# update component parameters for each repeat
onsetHWG = globalClock.getTime()
thisExp.addData('hwgOnset', onsetHWG)

thisExp.nextEntry()
# keep track of which components have finished
hwgComponents = [text]
for thisComponent in hwgComponents:
    if hasattr(thisComponent, 'status'):
        thisComponent.status = NOT_STARTED
...
facetrials = data.TrialHandler(nReps=1, method='random', 
    ...
    trialList=data.importConditions('RunB_ConditionsFile.csv'),
