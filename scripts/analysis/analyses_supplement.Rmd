...
datadirectory <- '' # put path to repository/data_new_IDs_anonymized here.
data <- read.csv(glue('{datadirectory}/behavior_rsa_conditionlevel_shortdata.csv'),stringsAsFactors = TRUE) # Read in data.

fullsample_questionnairedata <- read.csv(glue('{datadirectory}/fullsample_questionnairedata.csv'),stringsAsFactors = TRUE) # Read in data.

fullsample_long_questionnairedata <- read.csv(glue('{datadirectory}/fullsample_long_questionnairedata.csv'), header = TRUE)
...
timepoint_data <- read.csv(glue('{datadirectory}/quarantine_timepoint_data.csv'), header = TRUE)
...
print_multilevel_logistic_table <- function(mlm_log_model,model.terms.labels,region){
  ...
  model_df <- model_df %>% select("term","estimate","std.error","CI","statistic","p.value")
  model_df <- model_df %>% mutate(term = model.terms.labels)
  colnames(model_df) <- c("variable","odds ratio", "SE", "CI","z","p") # Note: odds ratio instead of estimate, statistic is a z value.
...
summary_data <- fullsample_questionnairedata %>% select(Age, Sex, race_combined,Hispanic, Percent_Surprised_Negative, anxiety_total_T1,mstat_total_score)
...
sample_information_table <- summary_data %>%
                           tbl_summary(label = c(Percent_Surprised_Negative ~ "Negativity bias",
...
library(lmerTest)
modelone_month <-  lmer(anxiety_total ~ participant_month + (1 +  participant_month | Participant_ID), data = fullsample_long_questionnairedata, REML = TRUE)
...
# Control for  time of quarantine onset relative to a participant’s timeline (a between-subjects factor, corresponding to whether the quarantine began between their T1 and T2 timepoints, or between their T2 and T3 timepoints, etc.).
...
print_multilevel_table(interaction_mstat_longitudinal_anxiety,
                        c("Intercept","MSTAT score (z)","Time (months)","Time x MSTAT score (z)"), # The order is not the same as they are entered in the model or how it is printed in #summary(); I think it is alphabethical?
                       "Anxiety score")

```

# Self-reported ambiguity tolerance and behavior in post-scan task.
## *Supplemental Table 6: Self-reported ambiguity tolerance and negativity biases.*
...
## *Supplemental Table 7: Negativity biases and baseline anxiety scores.*
...
## *Supplemental Table 8: Negativity biases and longitudinal anxiety scores.*
...
print_multilevel_table(anxiety_negativity_bias_longitudinal,
                        c("Intercept","Time (months)","Time x Negativity bias","Negativity bias (z)"), # The order is not the same as they are entered in the model or how it is printed in #summary(); I think it is alphabethical?
                       "Anxiety score")
```

## *Supplemental Table 9: Baseline anxiety and average reaction time to ambiguous stimuli.*
...
## *Supplemental Table 10: Ambiguous/threatening RS and baseline anxiety.*
### Right amygdala.
...
## *Supplemental Table 11: Ambiguous/nonthreatening RS and baseline anxiety.*
### Right amygdala.
...
## *Supplemental Table 12: Threatening/nonthreatening RS and baseline anxiety.*
### Right amygdala.
...
## *Supplemental Table 13: Ambiguous/threatening RS and longitudinal anxiety.*
### Right amygdala.
...
## *Supplemental Table 14: Ambiguous/nonthreatening RS and longitudinal anxiety.*
### Right amygdala.
...
## *Supplemental Table 15: Threatening/nonthreatening RS and longitudinal anxiety.*
### Right amygdala.
...
## *Supplemental Table 16: Actor-level ambiguous/threatening RS and subsequent appraisals of the actor's ambiguous image.*
### Right amygdala.
...
## *Supplemental Table 17: Actor-level ambiguous/nonthreatening RS and subsequent appraisals of the actor's ambiguous image.*
### Right amygdala.
...
## *Supplemental Table 18: Actor-level threatening/nonthreatening RS and subsequent appraisals of the actor's ambiguous image.*
### Right amygdala.
...
## *Supplemental Figure 1a: Visualization of study timepoints.*
...
## *Supplemental Figure 1b: Individual anxiety scores over time.*
...
## *Supplemental Figure 2: Average RSA values within the right and left amygdala by comparison type.*
...
## *Supplemental Figure 3: Negativity biases in sample. *
...
## *Supplemental Figure 4: Percentage of participants that categorized each actor’s surprised face negatively.*
...
# Plot percent of participants categorized each actor’s surprised face as negative.
...
## *Supplemental Figure 5: Reaction times to each actor's surprised face.*
```{r actor_reaction_times,echo=FALSE, warning = FALSE}
# Plot means and standard deviations of reaction times (on surprised trials) for each actor.
actor_rt_ratings_plot <- ggplot(actor_surprised_images,aes(x = reorder(Actor,image_taskbased_rt_average), y = image_taskbased_rt_average)) + 
  geom_bar(stat="identity",fill="gray80",alpha=0.5) +
...
actor_rt_ratings_plot

```

## *Supplemental Figure 6: Distribution of RS values by condition comparison type and by actor.*
...
actor_rsa_long <- actor_rsa_long %>% select(-c(Actor.Emotion))
