...
datadirectory <- 'https://github.com/nsaragosaharris/anxiety_ambiguity_study/tree/main/data_new_IDs_anonymized' # your directory here; putting in link to github for now (but make sure to edit this, as this will likely not work as it is).

# Read in data.
demographics <- read.csv(glue('{datadirectory}/demographics.csv',stringsAsFactors = TRUE))
demographics <- demographics %>% select(Participant_ID,Sex,Age,race_combined,Hispanic,firstgeneration)

data <- read.csv(glue('{datadirectory}/behavior_rsa_conditionlevel_shortdata.csv'),stringsAsFactors = TRUE)

fullsample_questionnairedata <- read.csv(glue('{datadirectory}/fullsample_questionnairedata.csv'), header = TRUE)
# Add first generation status to questionnaire data.
firstgeneration <- demographics %>% select(Participant_ID,firstgeneration)
fullsample_questionnairedata <- merge(fullsample_questionnairedata,firstgeneration,by="Participant_ID")
fullsample_questionnairedata$quarantine_onset_factor <- as.factor(fullsample_questionnairedata$quarantine_onset_factor)
...
print_multilevel_logistic_table <- function(mlm_log_model,model.terms.labels,region){
  ...
  model_df <- model_df %>% mutate(term = model.terms.labels)
  colnames(model_df) <- c("variable","odds ratio", "SE", "CI","z","p") # odds ratio instead of estimate, statistic is a z value.
...
modelone_month_control_quarantine_timepoint <-  lmer(anxiety_total ~ participant_month +  quarantine_onset_factor + (1 +  participant_month | Participant_ID), data = fullsample_long_questionnairedata, REML = TRUE)
...
## *Supplemental Figure 5: Percentage of participants that categorized each actor’s surprised face negatively.*
